...
public class ActionGraphLoader : LoaderBase<ActionGraphBaseData>
{
    ...
    public override ActionGraphBaseData readFromXML(string path)
    {
        ...
        foreach(var item in actionCompareDic)
        {
            ...
            {
                DebugUtil.assert_fileOpen(false,"target action is not exists : {0} [FileName: {1}]", _currentFileName, XMLScriptConverter.getLineNumberFromXMLNode(node),string.IsNullOrEmpty(item.Value) ? "NULL" : item.Value, _currentFileName);
                return null;
            }
        ...
        for(int attrIndex = 0; attrIndex < actionAttributes.Count; ++attrIndex)
        {
            ...
            else if(targetName == "Preset")
            {
                ...
                if(animationCustomPreset._rotationPresetName != "")
                {
                    ...
                    {
                        DebugUtil.assert_fileOpen(false,"로테이션 프리셋 로드 실패 [Preset: {0}] [FilePath: {1}]",filePath,XMLScriptConverter.getLineNumberFromXMLNode(node),animationCustomPreset._rotationPresetName,filePath);
                    }
                }
                ...
                if(animationCustomPreset._scalePresetName != "")
                {
                    ...
                    {
                        DebugUtil.assert_fileOpen(false,"스케일 프리셋 로드 실패 [Preset: {0}] [FilePath: {1}]",filePath,XMLScriptConverter.getLineNumberFromXMLNode(node),animationCustomPreset._scalePresetName,filePath);
                    }
                }
                ...
                if(animationCustomPreset._translationPresetName != "")
                {
                    ...
                    {
                        DebugUtil.assert_fileOpen(false,"트렌슬레이션 프리셋 로드 실패 [Preset: {0}] [FilePath: {1}]",filePath,XMLScriptConverter.getLineNumberFromXMLNode(node),animationCustomPreset._translationPresetName,filePath);
                    }
                }
